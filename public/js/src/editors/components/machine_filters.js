...
(function() {
  ...
  App.Components.MachineFilters = App.Components.ModalDialog.extend({
    template: JST['editors/machine_filters'],
    ...
    initialize: function(options) {
      ...
      options.id = "machine_filters";
      options.left = 800;
      options.top = 700;
      options.width = 640;
      options.height = 320;
      options.title = i18n.t("common.dialog.machine_filters");

      this._super(options);

      this.$el.addClass("machine-filters");

      this.editor = options.editor;
      this.machine_group = options.machine_group;
      this.machine_group.set('filters', this.machine_group.get('filters') || []);
      ...
      _.each(this.editor.graph.getElements(), function(cell) {
        ...
        if(cell === this.machine_group) { return; }
    ...
    render: function() {
      ...
      //  select系の値を画面に反映
      var filters = this.machine_group.get('filters');
      _.each(filters, function(filter, index) {
        if(filter.reference) {
          ...
          if(reference.opponent === App.Components.MachineFilters.RoutingKeywordsType.all) {
            this.$('.opponent select').eq(index).val(App.Components.MachineFilters.RoutingKeywordsType.all).attr('disabled', 'disabled');
          ...
          if(filter.opponent === App.Components.MachineFilters.RoutingKeywordsType.all) {
            this.$('.opponent select').eq(index).val(App.Components.MachineFilters.RoutingKeywordsType.all);
    ...
    onchange: function(e) {
      ...
      if(index >= filters.length) {
        ...
        row.after(this.partial('editors/_machine_filters_new'));
      }
      ...
      if(target.attr('name') === 'id' && target.prop('tagName') === 'SELECT') {
        ...
        if(reference) {
          ...
          if(reference.opponent === 'all') {
            this.$("tbody tr .opponent select").eq(index).val('all');
          } else {
        ...
        if(target.val() === App.Components.MachineFilters.RoutingKeywordsType.all) {
          filters[index][target.attr('name')] = App.Components.MachineFilters.RoutingKeywordsType.all;
        } else {
  ...
  App.Components.MachineFilters.ProtocolType = {
    tcp: "tcp",
    udp: "udp",
    icmp: "icmp"
  };

  App.Components.MachineFilters.DirectionType = {
    ingress: "ingress",
    egress: "egress"
  };

  App.Components.MachineFilters.RoutingKeywordsType = {
    all: "all",
    InternetGateway: "{{InternetGateway}}"
  };

  App.Components.MachineFilters.RuleActionType = {
    allow: "allow",
    deny: "deny"
  };
